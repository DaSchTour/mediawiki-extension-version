 ...
 * @ingroup SpecialPage
 */
class SpecialExtensionVersion extends SpecialPage {
	...
	function getExtensionCredits() {
		...
		if ( count( $wgExtensionFunctions ) ) {
			...
			$out .= '<tr><td colspan="5">' . $this->listToText( $wgExtensionFunctions ) . "</td></tr>\n";
		}
		...
		if ( $cnt ) {
			...
			$out .= '<tr><td colspan="5">' . $this->listToText( $tags ) . "</td></tr>\n";
		}

		$fhooks = $wgParser->getFunctionHooks();
		if ( count( $fhooks ) ) {
			$out .= $this->openExtType( $this->msg( 'version-parser-function-hooks' )->text(), 'parser-function-hooks' );
			$out .= '<tr><td colspan="5">' . $this->listToText( $fhooks ) . "</td></tr>\n";
		}
		*/
		...
		if ( isset( $extension['path'] ) ) {
			...
			if ( $gitHeadSHA1 !== false ) {
				...
				if ( $gitViewerUrl !== false ) {
					$vcsText = "$branchText<td>[$gitViewerUrl $vcsText]</td>";
				}
				else {
...
class ExtendedGitInfo extends GitInfo {
	public function getPullDate() {
		...
		if ( !is_readable( $LOGfile ) ) {
			return "unreadable" . $this->basedir;
		}
